...
class CategoryCubit extends Cubit<CategoryState> {
  ...
  CategoryCubit(this.categoriesUseCase,this.productsUseCase) : super(CategoryState(cartItems: {}));
  ...
  Future<void> getProductByCategory({bool refresh = false,int mainCategoryId=0}) async {
    emit(state.copyWith(productStatus: ProductStatus.loading));
    final page = refresh ? 1 : state.productCurrentPage;
    final result = await productsUseCase.getProductsByCategory(PaginationParams(page: page,mainCategoryId: mainCategoryId));
    result.fold(
          (failure) =>emit(state.copyWith(productStatus: ProductStatus.failure,errorMessage: failure.message)),
          (products) =>emit(state.copyWith(productStatus: ProductStatus.success,products:products.items)),
    );
  }
  ...
  void increaseQuantity(String productId) {
    final updatedItems = List<Product>.from(state.products);
     ...
     int productIndex= updatedItems.indexWhere((element) => element.id.toString() == productId);
